...
const createProduct = async (req, res) => {
    let {nombre, descripcion, numero, imagen, imagenAlt, id_categoria, categoria, precio, stock} = req.body;
    const all = await classProducts.getAll();

    const id = all.length + 1;

    if ((nombre !== undefined) && (descripcion !== undefined) && (numero !== undefined) && (imagen !== undefined) && (imagenAlt !== undefined) && (id_categoria !== undefined) && (categoria !== undefined) && (precio !== undefined) && (stock !== undefined)) {
        ...
        descripcion = createArray(descripcion);
        numero = numero.split(", ");
        let numeroNumber = numero.map(t => {
            return Number(t);
        });
        ...
        res.status(206).json("Todos los campos son obligatorios, vuelve a intentarlo");
    };    
};
